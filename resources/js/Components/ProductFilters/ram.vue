<template>
    <div class="p-4 bg-white shadow-md rounded-lg">
        <p class="font-bold text-lg cursor-pointer" @click="toggleVisibility">Memory</p>
...
const capacities = props.ram || [];
const showMore = ref(false);
const isChanged = ref(false);

const capacitiesValue = ref(new URLSearchParams(window.location.search).get('ram'));
...
function closeAll() {
    checkedCapacities.value = []; 
}

function toggleVisibility() {
    isVisible.value = !isVisible.value; // Toggle the visibility state
}
